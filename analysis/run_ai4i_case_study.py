...
def main() -> None:
    ...
    model_selection = {
        ...
        "model_card": build_model_card(df, DATASET_NAME, DATASET_DOI, RANDOM_SEED),
        "selection_reason": (
        ...
        "next_step": (
            "Done: AI4I 2020 is a static snapshot dataset with no run-to-failure time series, so it "
            "cannot support real RUL (remaining-useful-life) regression. Genuine RUL regression is "
            "now demonstrated in a separate, clearly-labeled case study on NASA's C-MAPSS "
            "run-to-failure dataset — see docs/rul-case-study.md and /rul-case-study."
        ),
    }
